...
int main() {
    ...
    //const char* outputFile = "output_grayscale-c.bmp";
    //const char* edgeFile = "output_edge-c.bmp";
    //const char* memFile = "output_image-c.mem";
    //const char* edgeMemFile = "output_edge-c.mem";
    ...
    // 엣지 검출 MEM 파일 생성
    if (fopen_s(&edgeMemOutFile, edgeMemFile, "w") != 0 || edgeMemOutFile == NULL) {
        printf("엣지 MEM 파일을 생성할 수 없습니다: %s\n", edgeMemFile);
        free(imageData);
        free(grayscaleData);
        free(edgeData);
        return 1;
    ...
    // MEM 파일에 엣지 검출 데이터를 16진수로 쓰기
    for (int i = 0; i < IMAGE_WIDTH * IMAGE_HEIGHT; i++) {
        fprintf(edgeMemOutFile, "%02X\r\n", edgeData[i]);
    }

    fclose(edgeMemOutFile);
    ...
    printf("엣지 검출 MEM 파일: %s\n", edgeMemFile);
    printf("모든 파일이 성공적으로 생성되었습니다.\n");
